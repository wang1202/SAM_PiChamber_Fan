...
real, allocatable, dimension(:,:,:) :: fluxlmk, fluxrmk !left/right fluxes
real, allocatable, dimension(:,:,:) :: fluxqmk, fluxhmk !front/back fluxes
...
!bloss/qt: cloud liquid water no longer prognosed
  ...
  if(.not.isallocatedMICRO) then
     ...
     allocate(micro_field(dimx1_s:dimx2_s,dimy1_s:dimy2_s,nzm,nmicro_fields), &
          ...
          fluxlmk(ny,nzm,nmicro_fields), fluxrmk(ny,nzm,nmicro_fields), &
          fluxqmk(nx,nzm,nmicro_fields), fluxhmk(nx,nzm,nmicro_fields), &
...
!   change for pi chamber
      if (docloud) call satadj_liquid(nzm,tabs0,q0,qc0,pres*100.)
...
!     if(docloud) call micro_diagnose()   ! leave this here
     call micro_diagnose() ! debug for pi chamber
...
!if (fluxbmk(10,10,index_water_vapor)>0) then
!print*,'debug positive',fluxbmk(10,10,index_water_vapor),fluxtmk(10,10,index_water_vapor)
!stop
!end if

!if (fluxbmk(10,10,index_water_vapor)<0) then
!print*,'debug negative',fluxbmk(10,10,index_water_vapor),fluxtmk(10,10,index_water_vapor)
!stop
!end if
