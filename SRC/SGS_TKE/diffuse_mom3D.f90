...
use params, only: docolumn, dowallx, dowally, dowallmflux
implicit none
...
!-----------------------------------------------------------
! left/right wall flux of momemtum for pi chamber
! FY @ MTU 2017
if (dowallmflux) then
 if (dowallx) then
  if (mod(rank,nsubdomains_x).eq.0) then
! left wall
   fu(0,j,k)=fu(1,j,k)
   fv(0,j,k)=fluxlv(j,k)/dx
   fw(0,j,k)=fluxlw(j,k)/dx
  end if
  if (mod(rank,nsubdomains_x).eq.nsubdomains_x-1) then
! right wall
   fu(nx,j,k)=-2.*rdx21*tk(nx,j,k)*(-u(nx,j,k))
   fv(nx,j,k)=fluxrv(j,k)/dx
   fw(nx,j,k)=fluxrw(j,k)/dx
  end if
 end if ! end of dowallx
end if ! end of dowallmflux
...
!-----------------------------------------------------------
! front/back wall flux of momemtum for pi chamber
! FY @ MTU 2017
if (dowallmflux) then
do i=1,nx
 if (rank.lt.nsubdomains_x) then
! front wall
  fv(i,0,k)=fv(i,1,k)
  fw(i,0,k)=fluxqw(i,k)/dy
  fu(i,0,k)=fluxqu(i,k)/dy
 end if
 if (rank.gt.nsubdomains-nsubdomains_x-1) then
! back wall
  fv(i,ny,k)=-2.*rdy21*tk(i,ny,k)*(-v(i,ny,k))
  fw(i,ny,k)=fluxhw(i,k)/dy
  fu(i,ny,k)=fluxhu(i,k)/dy
 end if
end do
end if
...
!   fw(i,j,nz)=-2.*tkz*(w(i,j,nz)-w(i,j,nzm))/adz(nzm)*rho(nzm)
   fw(i,j,nz)=-2.*tkz*(-w(i,j,nzm))/adz(nzm)*rho(nzm) ! change for pi chamber -- fan
