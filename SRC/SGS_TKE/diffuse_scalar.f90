...
subroutine diffuse_scalar (f,fluxb,fluxt,fluxl,fluxr,fluxq,fluxh, &
                          fdiff,flux,f2lediff,f2lediss,fwlediff,doit)
...
real fluxl(ny,nzm)		! left flux
real fluxr(ny,nzm)		! right flux
real fluxq(nx,nzm)		! front flux
real fluxh(nx,nzm)		! back flux
real flux(nz)
real fdiff(nz)
real f2lediff(nzm)
real f2lediss(nzm)
...
if(RUN3D) then
  call diffuse_scalar3D (f,fluxb,fluxt,fluxl,fluxr,fluxq,fluxh, &
                           tkh,rho,rhow,flux)
!  call diffuse_scalar3D (f,fluxb,fluxt,tkh,flux)
else  
