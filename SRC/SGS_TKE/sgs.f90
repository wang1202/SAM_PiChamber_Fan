...
subroutine sgs_scalars()
  ...
  implicit none
    ...
    real fluxltmp(ny,nz), fluxrtmp(ny,nz)
    real fluxqtmp(nx,nz), fluxhtmp(nx,nz)
    integer k
...
! add for pi chamber
! FY @ MTU 2017
      call diffuse_scalar(t,fluxbt,fluxtt,fluxlt,fluxrt, &
                            fluxqt,fluxht,tdiff,twsb, &      ! AW: bug, fluxhq -> fluxht
                           t2lediff,t2lediss,twlediff,.true.)
      ...
      do k = 1,nmicro_fields
        if(   k.eq.index_water_vapor             &! transport water-vapor variable no metter what
         ...
         .or. doprecip.and.flag_precip(k).eq.1 ) then
           ...
           fluxltmp(1:ny,1:nz) = fluxlmk(1:ny,1:nz,k)
           fluxrtmp(1:ny,1:nz) = fluxrmk(1:ny,1:nz,k)
           fluxqtmp(1:nx,1:nz) = fluxqmk(1:nx,1:nz,k)
           fluxhtmp(1:nx,1:nz) = fluxhmk(1:nx,1:nz,k)
           call diffuse_scalar(micro_field(:,:,:,k),fluxbtmp,fluxttmp, &
                fluxltmp,fluxrtmp,fluxqtmp,fluxhtmp, &
                mkdiff(:,k),mkwsb(:,k), dummy,dummy,dummy,.false.)
       end if
      end do
      total_water_evap = total_water_evap + total_water()

 ! diffusion of tracers: not used in pi chamber in this version
