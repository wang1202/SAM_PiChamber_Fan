...
real def2(nx,ny,nzm)	
real grd,betdz,Ck,Ce,Ces,Ce1,Ce2,smix,Pr,Cee,Cs,Le
...
!   Pr=1. +2.*ratio
   ...
   if(dosmagor) then
     ...
     a_prod_sh=(tk(i,j,k)+0.0000001)*def2(i,j,k)
     a_prod_bu=-(tk(i,j,k)+0.0000001)*Pr*buoy_sgs
     a_diss=a_prod_sh+a_prod_bu

   else

     tke(i,j,k)=max(0.,tke(i,j,k))
     a_prod_sh=(tk(i,j,k)+0.000001)*def2(i,j,k)
     a_prod_bu=-(tk(i,j,k)+0.0000001)*Pr*buoy_sgs
