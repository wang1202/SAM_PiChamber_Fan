subroutine compress3DSWqh (f,nx,ny,nz,name, long_name, units, &
...
! Modified for different MPI data gathering at the front(q: qian)/back(h: hou) side walls. Aaron Wang @ PNNL
...
character(10) value_min(ny), value_max(ny)
character(7) form
...
if(savebin) then	
  ...
  if(SW.eq.1) then ! front wall
    ...
    if(output_sep) then ! output_sep is not tested, yet
      ...
      do irank = 1, nsubdomains_x
        irank2 = nsubdomains - nsubdomains_x - 1 + irank
        call task_barrier()
        if(irank2.eq.rank) then
          call task_bsend_float4(0,byte4,count,irank2)
        end if
