...
!   add for pi chamber move to statistics
!real qvsat   (nx, ny, nzm)                ! saturated water vapor mixing ratio
!real ss      (nx, ny, nzm)                ! saturation ratio
...
! fluxes at the left and right wall of the domain for pi chamber -- FY
! FY @ MTU 2017
...
! add for pi chamber: vars for detector
! FY @ MTU 2017
integer, allocatable :: plocation_rtd(:,:) ! index for detector
real, allocatable :: det_rtd(:,:,:) ! index for variable
integer, allocatable :: plocation_sonicu(:,:) ! index for detector
real, allocatable :: det_sonicu(:,:,:) ! index for variable
integer, allocatable :: plocation_sonicv(:,:) ! index for detector
real, allocatable :: det_sonicv(:,:,:) ! index for variable
integer, allocatable :: plocation_sonicw(:,:) ! index for detector
real, allocatable :: det_sonicw(:,:,:) ! index for variable
integer, allocatable :: plocation_licor(:,:) ! index for detector
real, allocatable :: det_licor(:,:,:) ! index for variable
integer, allocatable :: plocation_thermistors(:,:) ! index for detector
real, allocatable :: det_thermistors(:,:,:) ! index for variable
integer, allocatable :: plocation_virtual(:,:) ! index for detector
real, allocatable :: det_virtual(:,:,:) ! index for variable
!---------------------------------------------------------------------
...
real sttxy(0:nx,(1-YES3D):ny)	!  surface temperature xy-distribution (perturbation from t00)
...
real shf_xy(nx,ny)              ! shf xy-distribution
real lhf_xy(nx,ny)              ! lhf xy-distribution
! Top and side wall's flux. Aaron Wang @ PNNL
real shft_xy(nx,ny)              ! shf xy-distribution at the top wall
real lhft_xy(nx,ny)              ! lhf xy-distribution at the top wall
real shfl_yz(ny,nzm)              ! shf yz-distribution at the left wall
real lhfl_yz(ny,nzm)              ! lhf yz-distribution at the left wall
real shfr_yz(ny,nzm)              ! shf yz-distribution at the right wall
real lhfr_yz(ny,nzm)              ! lhf yz-distribution at the right wall
real shfq_xz(nx,nzm)              ! shf xz-distribution at the front wall
real lhfq_xz(nx,nzm)              ! lhf xz-distribution at the front wall
real shfh_xz(nx,nzm)              ! shf xz-distribution at the back wall
real lhfh_xz(nx,nzm)              ! lhf xz-distribution at the back wall
