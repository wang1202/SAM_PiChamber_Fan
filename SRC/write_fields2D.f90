...
character *120 filenameSWlr ! Added for an additional 2D output for left/right SW.  Aaron Wang @ PNNL
character *120 filenameSWqh ! Added for an additional 2D output for front/back SW.  Aaron Wang @ PNNL
...
character *10 units
character *12 c_dx, c_dy, c_dz, c_time
integer i,j,k,nfields,nfields1,nsteplast
real(4) tmp(nx,ny,1), tmp_lr(1,ny,nzm), tmp_qh(nx,1,nzm)
real(4) dummy
character*7 filestatus
real coef
logical, save :: notopend2D=.true.  ! Aaron Wang: this flag is not used? (different from notopened2D)

nfields= 30+10 ! Aaron Wang: plus top and side walls SHF and LHF.  
               ! Left, right, front, and back wall's fluxes must be the last 8 fields,
               ! or this file and UTIL need to be modified.
       ...
       do while(.true.)
         ...
         do i=1,nfields
           ...
           if(.not.output_sep) then
             if (i .le. nfields - 8) then
               do j=1,nsubdomains-1
                 read(46)
               end do
             else if (i .gt. nfields - 8 .and. i .le. nfields - 4) then
               do j=1,nsubdomains_y-1
                 read(46)
               end do
             else
               do j=1,nsubdomains_x-1
                 read(46)
               end do
             end if 
           end if
...
if(save2Davg) then ! This function is not available for side walls, yet.  Aaron Wang @ PNNL
...
if(.not.SFC_FLX_FXD) then
  ...
  name='SHF'
  long_name='Sensible Heat Flux at Bottom'
  units='W/m2'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
   nfields1=nfields1+1
  ...
  name='LHF'
  long_name='Latent Heat Flux at Bottom'
  units='W/m2'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
end if
...
! END UW ADDITIONS
!=====================================================
! Wall fluxes added for Pi Chamber. Aaron Wang @ PNNL
! Top wall
   nfields1=nfields1+1
   do j=1,ny
    do i=1,nx
...
! Left wall
  ...
  name='SHFL'
  long_name='Sensible Heat Flux at Left SW (dim: y, z)'
  units='W/m2'
  call compress3DSWlr(tmp_lr,1,ny,nzm,name,long_name,units, &
                      save2Dbin,dompi,rank,nsubdomains,nsubdomains_x,nsubdomains_y,1)
   nfields1=nfields1+1
   do k=1,nzm
    do j=1,ny
  ...
  name='LHFL'
  long_name='Latent Heat Flux at Left SW (dim: y, z)'
  units='W/m2'
  call compress3DSWlr(tmp_lr,1,ny,nzm,name,long_name,units, &
                      save2Dbin,dompi,rank,nsubdomains,nsubdomains_x,nsubdomains_y,1)
! Right wall
  ...
  name='SHFR'
  long_name='Sensible Heat Flux at Right SW (dim: y, z)'
  units='W/m2'
  call compress3DSWlr(tmp_lr,1,ny,nzm,name,long_name,units, &
                      save2Dbin,dompi,rank,nsubdomains,nsubdomains_x,nsubdomains_y,2)
   nfields1=nfields1+1
   do k=1,nzm
    do j=1,ny
  ...
  name='LHFR'
  long_name='Latent Heat Flux at Right SW (dim: y, z)'
  units='W/m2'
  call compress3DSWlr(tmp_lr,1,ny,nzm,name,long_name,units, &
                      save2Dbin,dompi,rank,nsubdomains,nsubdomains_x,nsubdomains_y,2)
! Front wall
  ...
  name='SHFQ'
  long_name='Sensible Heat Flux at Front SW (dim: x, z)'
  units='W/m2'
  call compress3DSWqh(tmp_qh,nx,1,nzm,name,long_name,units, &
                      save2Dbin,dompi,rank,nsubdomains,nsubdomains_x,nsubdomains_y,1)
   nfields1=nfields1+1
   do k=1,nzm
    do i=1,nx
  ...
  name='LHFQ'
  long_name='Latent Heat Flux at Front SW (dim: x, z)'
  units='W/m2'
  call compress3DSWqh(tmp_qh,nx,1,nzm,name,long_name,units, &
                      save2Dbin,dompi,rank,nsubdomains,nsubdomains_x,nsubdomains_y,1)
! Back wall
  ...
  name='SHFH'
  long_name='Sensible Heat Flux at Back SW (dim: x, z)'
  units='W/m2'
  call compress3DSWqh(tmp_qh,nx,1,nzm,name,long_name,units, &
                      save2Dbin,dompi,rank,nsubdomains,nsubdomains_x,nsubdomains_y,2)
   nfields1=nfields1+1
  ...
  name='LHFH'
  long_name='Latent Heat Flux at Back SW (dim: x, z)'
  units='W/m2'
  call compress3DSWqh(tmp_qh,nx,1,nzm,name,long_name,units, &
                      save2Dbin,dompi,rank,nsubdomains,nsubdomains_x,nsubdomains_y,2)
