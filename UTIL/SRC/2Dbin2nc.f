...
c Modified for outputing the side wall's fluxes with different 2D domain.  Aaron Wang @ PNNL
c
...
c Added for left/right (lr) and front/back (q/h) side walls.  Aaron Wang @ PNNL
	real(4), allocatable :: byte_lr(:)
	real(4), allocatable :: fld_lr(:)
	real(4), allocatable :: byte_qh(:)
	real(4), allocatable :: fld_qh(:)

	real(4) fmax,fmin,day0
	real(8) dx,dy,dz,x(100000),y(100000),z(100000),time
	logical condition
	integer nsubs,nsubsx,nsubsy,nx,ny,nz,nfields,nstep
	integer i,j,k,k1,k2,n,i0,j0,nx_gl,ny_gl,length,ifields

	integer vdimids(3),vdimids_lr(3),vdimids_qh(3),start(3),count(3),count_lr(3),count_qh(3),ndimids,nfile
	integer ncid,err,yid,xid,zid,timeid,ntime,varrr
	integer nrecords
...
c Added for left/right (lr) and front/back (q/h) side walls.  Aaron Wang @ PNNL
          allocate(byte_lr(ny*nz))
	  allocate(fld_lr(ny_gl*nz))
          allocate(byte_qh(nx*nz))
	  allocate(fld_qh(nx_gl*nz))
        end if
c
...
c
	...
	err = NF_DEF_DIM(ncid, 'time', NF_UNLIMITED, timeid)
        ...
        err = NF_DEF_VAR(ncid, 'x', NF_DOUBLE, 1, xid, varrr)
	err = NF_PUT_ATT_TEXT(ncid,varrr,'units',1,'m')
	if(ny_gl.ne.1) then
         err = NF_DEF_VAR(ncid, 'y', NF_DOUBLE, 1, yid, varrr)
	 err = NF_PUT_ATT_TEXT(ncid,varrr,'units',1,'m')
	endif
		err = NF_DEF_VAR(ncid, 'z', NF_DOUBLE, 1, zid, varrr)
	err = NF_PUT_ATT_TEXT(ncid,varrr,'units',1,'m')
...
c   The last 8 fields need to be allocated differently.  Aaron Wang @ PNNL
		if (ifields .le. nfields - 8) then
			do n=0,nsubs-1		
		...
		else if (ifields .gt. nfields - 8 .and. ifields .le. nfields - 4) then
			do n=0,nsubsy-1
		
				read(1) (byte_lr(k),k=1,ny*nz)
				i0 = n * (ny_gl/nsubsy)
				...
				do j=1,nz
					do i=1+i0,ny+i0
						length=length+1
						fld_lr(i+ny_gl*(j-1))=byte_lr(length) 
					end do
				end do
			...
			if(condition) then
				err = NF_REDEF(ncid)
				err = NF_DEF_VAR(ncid,name,NF_FLOAT,ndimids,vdimids_lr,varrr)
				err = NF_PUT_ATT_TEXT(ncid,varrr,'long_name',strlen1(long_name),long_name(1:strlen1(long_name)))
				err = NF_PUT_ATT_TEXT(ncid,varrr,'units',strlen1(units),units(1:strlen1(units)))
				err = NF_ENDDEF(ncid)
			end if
			...
			err = NF_INQ_VARID(ncid,name,varrr)
			err = NF_PUT_VARA_REAL(ncid,varrr,start,count_lr,fld_lr)
		else
			do n=0,nsubsx-1
			
				read(1) (byte_qh(k),k=1,nx*nz)
				...
				do j=1,nz
					do i=1+i0,nx+i0
						length=length+1
						fld_qh(i+nx_gl*(j-1))=byte_qh(length) 
					end do
				end do
			end do ! n
			if(condition) then
				err = NF_REDEF(ncid)
				err = NF_DEF_VAR(ncid,name,NF_FLOAT,ndimids,vdimids_qh,varrr)
				err = NF_PUT_ATT_TEXT(ncid,varrr,'long_name',strlen1(long_name),long_name(1:strlen1(long_name)))
				err = NF_PUT_ATT_TEXT(ncid,varrr,'units',strlen1(units),units(1:strlen1(units)))
				err = NF_ENDDEF(ncid)
			end if
			...
			err = NF_INQ_VARID(ncid,name,varrr)
			err = NF_PUT_VARA_REAL(ncid,varrr,start,count_qh,fld_qh)
		end if
	end do
