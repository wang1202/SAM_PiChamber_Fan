...
c variables:
	...
	real(4) fmax,fmin
	real(8) dx,dy,z(500),p(500),x(100000),y(100000),time
        real(8) rho(500)
        real(8) rccn(33), dropr(33) !remember change the number here
        real(8) test1,test2
...
c
	...
	read(1) nx,ny,nz,ncn,ncd,nsubs,nsubsx,nsubsy,nfields
...
c
	...
	err = NF_DEF_DIM(ncid, 'bina', ncn, binida)
	err = NF_DEF_DIM(ncid, 'binc', ncd, binidc)
	err = NF_DEF_DIM(ncid, 'time', NF_UNLIMITED, timeid)

        err = NF_DEF_VAR(ncid, 'x', NF_DOUBLE, 1, xid, varid)
	err = NF_PUT_ATT_TEXT(ncid,varid,'units',1,'m')
	if(ny_gl.ne.1) then
         err = NF_DEF_VAR(ncid, 'y', NF_DOUBLE, 1, yid, varid)
	 err = NF_PUT_ATT_TEXT(ncid,varid,'units',1,'m')
	endif
        err = NF_DEF_VAR(ncid, 'z', NF_DOUBLE, 1, zid, varid)
	err = NF_PUT_ATT_TEXT(ncid,varid,'units',1,'m')
	...
	err = NF_PUT_ATT_TEXT(ncid,varid,'units',3,'day')
        ...
        err = NF_DEF_VAR(ncid, 'p', NF_DOUBLE, 1, zid,varid)
	err = NF_PUT_ATT_TEXT(ncid,varid,'units',2,'mb')
        err = NF_PUT_ATT_TEXT(ncid,varid,'long_name',8,'pressure')
        err = NF_DEF_VAR(ncid, 'RA', NF_DOUBLE, 1, binida,varid)
	err = NF_PUT_ATT_TEXT(ncid,varid,'units',1,'-')
        err = NF_PUT_ATT_TEXT(ncid,varid,'long_name',8,'CCNSpect')
        err = NF_DEF_VAR(ncid, 'RC', NF_DOUBLE, 1, binidc,varid)
	err = NF_PUT_ATT_TEXT(ncid,varid,'units',1,'-')
        err = NF_PUT_ATT_TEXT(ncid,varid,'long_name',9,'DropSpect')
       

	err = NF_ENDDEF(ncid)

	err = NF_INQ_VARID(ncid,'x',varid)
	err = NF_PUT_VAR_DOUBLE(ncid, varid, x)
	if(ny_gl.ne.1) then
	err = NF_INQ_VARID(ncid,'y',varid)
	err = NF_PUT_VAR_DOUBLE(ncid, varid, y)
	endif
	err = NF_INQ_VARID(ncid,'z',varid)
	err = NF_PUT_VAR_DOUBLE(ncid, varid, z)
	err = NF_INQ_VARID(ncid,'time',varid)
	err = NF_PUT_VAR1_DOUBLE(ncid, varid, 1, time)
	err = NF_INQ_VARID(ncid,'p',varid)
	err = NF_PUT_VAR_DOUBLE(ncid, varid, p)
	err = NF_INQ_VARID(ncid,'RA',varid)
	err = NF_PUT_VAR_DOUBLE(ncid, varid, rccn)
	err = NF_INQ_VARID(ncid,'RC',varid)
	err = NF_PUT_VAR_DOUBLE(ncid, varid, dropr)
